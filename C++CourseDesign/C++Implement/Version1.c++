...
/* 
 string a[]={"用户注册","用户登陆","搜索图书"};
    Button* button =new Button(a,3);
    button->showButtonInformation();
    button->listenButtonBePressed();
    cout<< button->GetButtonInfo();
    if (button->bePressed.compare("0")==0){
       IObserver* u1=new GeneralUserCreate();
       ExcuteInterfaceFunc(subject,u1);
       delete u1;
    }
 */

// page0--读者注册","读者登陆","管理员登陆
// page1--"搜索图书","借阅图书","归还书籍","预定书籍","取消预定","续借书籍"
...
int main(){ 
   ...
   /*  string a[]={"读者注册","读者登陆","管理员登陆"};
    ...
    App*  app = new App(subject,page);
    app->AppInit(a,3,funcList);
    if */ 
    ...
    /* else if (page->button->bePressed.compare("1")==0){
        ...
        page->PageRefresh(a,3,funcList);
    } */
    /* page->PageListen(); 
    cout<<page->button->bePressed;
    cout<<page->buttonfuncMap.size(); */
